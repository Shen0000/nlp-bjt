...
def load_paired_data(filepath):
    """Load pre-paired human/LLM data from JSONL file and process it for DetectGPT
    
    Args:
        filepath: Path to .json or .jsonl file with paired data
        
    Returns:
        Dict with 'original' (human) and 'sampled' (LLM) keys
    """
    print(f'Loading paired data from {filepath}...')
    
    # Load the paired data using custom_datasets
    paired_data = custom_datasets.load('news_jsonl', cache_dir, filepath=filepath)
    
    print(f'Loaded {len(paired_data)} paired examples')
    
    # Extract human and LLM texts
    human_texts = [item['human'] for item in paired_data]
    llm_texts = [item['llm'] for item in paired_data]
    
    # Process texts: strip newlines
    human_texts = [strip_newlines(text.strip()) for text in human_texts]
    llm_texts = [strip_newlines(text.strip()) for text in llm_texts]
    
    # Trim to shorter length and create processed data
    processed_data = {
        "original": [],
        "sampled": []
    }
    
    for human, llm in zip(human_texts[:n_samples], llm_texts[:n_samples]):
        # Trim both to the same length
        human, llm = trim_to_shorter_length(human, llm)
        processed_data["original"].append(human)
        processed_data["sampled"].append(llm)
    
    print(f"Processed {len(processed_data['original'])} paired samples")
    print(f"Average number of words (human): {np.mean([len(x.split()) for x in processed_data['original']])}")
    print(f"Average number of words (LLM): {np.mean([len(x.split()) for x in processed_data['sampled']])}")
    
    return processed_data


def generate_data(dataset, key):
    # Check if we're using paired data instead of generating
    if args.paired_data_path:
        return load_paired_data(args.paired_data_path)
...
def load_base_model_and_tokenizer(name):
    if args.openai_model is None:
        ...
        # Apply FP16 if base_half flag is set
        if args.base_half:
            base_model_kwargs.update(dict(torch_dtype=torch.float16))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--paired_data_path', type=str, default=None, help='Path to pre-paired JSON/JSONL file with human and LLM text')
