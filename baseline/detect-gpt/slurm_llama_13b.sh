#!/bin/bash
#SBATCH --job-name=detectgpt-llama13b
#SBATCH --partition=gpu-a100-q
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=100G
#SBATCH --time=12:00:00
#SBATCH --output=logs/detectgpt-llama13b-%j.out
#SBATCH --error=logs/detectgpt-llama13b-%j.err

# DetectGPT evaluation for Llama 13B news dataset
# Runs on A100 GPU with more memory for 13B model

set -e

echo "========================================"
echo "DetectGPT Evaluation - Llama 13B"
...
python run.py \
    ...
    --batch_size 10 \
    --dataset xsum \
    --paired_data_path "${DATA_DIR}/llama_13B_sampled.jsonl" \
    --cache_dir "${CACHE_DIR}" \
    --skip_baselines \
    --base_half

echo "Llama 13B evaluation completed"
