#!/bin/bash
#SBATCH --job-name=detectgpt-llama7b
#SBATCH --partition=gpu-a100-q
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=80G
#SBATCH --time=10:00:00
#SBATCH --output=logs/detectgpt-llama7b-%j.out
#SBATCH --error=logs/detectgpt-llama7b-%j.err
...
echo "========================================"
echo "DetectGPT Evaluation - Llama 7B"
...
python run.py \
    ...
    --batch_size 15 \
    --dataset xsum \
    --paired_data_path "${DATA_DIR}/llama_7B_sampled.jsonl" \
    --cache_dir "${CACHE_DIR}" \
    --skip_baselines \
    --base_half

echo "Llama 7B evaluation completed"
