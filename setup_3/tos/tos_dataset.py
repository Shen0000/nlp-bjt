...
class ToSDataset:
    ...
    def process_news_dataset(
        ...
        dataset_name: str, # --> change dataset name here
        ...
        for line in data:
            try:
                human = line["human"].strip()
                llm = line["llm"].strip()
            except IndexError:
                continue

            if len(human) < min_char_len or len(llm) < min_char_len:
                continue
            ...
            llm_sample = {"text": llm, "source": f"{source}", "label": 1}
        ...
        save_path = os.path.join(
            processed_save_dir, f"{source}.discourse_parsed.jsonl"
        )
    ...
    def process_original_dataset(
        ...
        with open(dataset_name, "r", encoding="utf-8") as f:    
            for line in f:
                try:
                    ...
                    original = sample["original"][0].strip()
                    translated = sample["text"][0].strip()
                except IndexError:
                    continue

                if len(original) < min_char_len or len(translated) < min_char_len:
                    continue
                ...
                translated_sample = {"text": translated, "source": f"{source}", "label": 0}
                dataset[source].append(original_sample)
                dataset[source].append(translated_sample)
        ...
        save_path = os.path.join(
            processed_save_dir, f"original.discourse_parsed.jsonl"
        )
        with open(save_path, "w") as file:
            for document in dataset[source]:
                file.write(f"{document.model_dump(mode='json')}\n")
    
    def process_translated_dataset(
        self,
        processed_save_dir: str,
        dataset_name: str, # --> change dataset name here
        min_char_len: int = 10,
        ...
        with open(dataset_name, "r", encoding="utf-8") as f:
            for line in f:
                ...
                translated_sample = {"text": translated, "source": f"{source}", "label": 1}
                dataset[source].append(original_sample)
                dataset[source].append(translated_sample)
        ...
        save_path = os.path.join(
            processed_save_dir, f"translated.discourse_parsed.jsonl"
        )
        with open(save_path, "w") as file:
            for document in dataset[source]:
                file.write(f"{document.model_dump(mode='json')}\n")
...
#                    continue
#
#                source = str(sample["src"][0]) # --> change between original and translated
#                sources.add(source)
#
#                original_sample = {
#                    "text": original,
...
#
#            save_path = os.path.join(
#                processed_save_dir, f"trans_{source}.discourse_parsed.jsonl"
#            )
#            with open(save_path, "w") as file:
#                for document in dataset[source]:
#                    file.write(f"{document.model_dump(mode='json')}\n")
